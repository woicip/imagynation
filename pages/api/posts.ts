...
export default function handler(req: NextApiRequest, res: NextApiResponse<{ date: Date, posts: Array<Posts> } | { status: string, message: string }>) {
    if(req.method !== 'GET') return res.status(405).json({ status: 'Error', message: 'Method Not Allowed' })   
    res.status(200).json({ date: new Date(), posts: posts })
}
