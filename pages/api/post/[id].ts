...
export default function handler(req: NextApiRequest, res: NextApiResponse<{ date: Date, post: Posts } | { status: string, message: string }>){
    ...
    const thePost = posts[id - 1]
    res.status(200).json({ date: new Date(), post: thePost })
}
